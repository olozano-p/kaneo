...
- Password reset requires SMTP. The "Forgot password?" link sits under the password field, so it only appears when the password form is shown (`DISABLE_EMAIL_OTP_SIGN_IN=true`, or no SMTP-backed code sign-in). Reset links expire after one hour, can be used once, and signing out every other session for that user is part of the reset. With email code sign-in left on, a forgotten password is not a lockout: the user signs in with a code instead.
